...
class HSTrainerFilterView: UIView, UITextFieldDelegate {
    ...
    @IBOutlet weak var zipCheckImageView: UIImageView!
    @IBOutlet weak var dateCheckImageView: UIImageView!
    @IBOutlet weak var classCheckImageView: UIImageView!
    @IBOutlet weak var samplingView: UIView!
...
//        classTableView.reloadData()
        ...
        //Fetch class Types with specialities api
        self.getAvailableClassTypes()
    }

    func getAvailableClassTypes() {
        if HSAppManager.shared.classTypes.count == 0 {
            HSLoadingView.standardLoading().startLoading()
            HSUserNetworkHandler().getSpecialities { (result) in
                HSLoadingView.standardLoading().stopLoading()
                switch result {
                case .success(let sps):
                    DispatchQueue.main.async {
                        var collections = [String]()
                        sps.specialities?.forEach({ (sp) in
                            collections.append(sp.name!)
                        })
                        self.classTypes = collections
                        HSAppManager.shared.classTypes = self.classTypes
            ...
            self.classTypes = HSAppManager.shared.classTypes
            self.classTableView.reloadData()
        }
    }
