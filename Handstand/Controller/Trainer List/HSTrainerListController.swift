...
class HSTrainerListController: HSBaseController, HSTrainerListHeaderViewDelegate, UITableViewDelegate,UITableViewDataSource {
    
    @IBOutlet weak var filterBaseView: UIView!
    @IBOutlet weak var tableView: UITableView!
    let cellIdentifier = "trainerCell"
    var trainers : Array<HSTrainer>! = []
    weak var filterView : HSTrainerFilterView?
    weak var selectedAddress : HSAddress?
    var selectedImage : UIImageView! = nil
    weak var footerView : HSTrainerListFooterView?
    var offset = 0
    let offset_Value = 20
    ...
    func setupUI()  {
        ...
        //Header
        if let code = selectedAddress?.zipCode {
            self.title =  "(" + code + ")" + " " + "Trainers"
        }
...
//    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        if trainers.count > indexPath.row {
            ...
            cell.roundRectView.imageView.setImage(url: theTrainer.avatar, style:.rounded)
