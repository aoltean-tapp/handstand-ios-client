...
//        imageView.setImage(url: trainer.avatar, style: .rounded)
        let roundedView = HSRoundedView.init(frame: CGRect(x:(ScreenCons.SCREEN_WIDTH-120)/2,y:(topView.frame.height-120)/2,width:120,height:120))
        roundedView.ivHeight = 118
        roundedView.ivWidth = 118
        ...
        self.topView.addSubview(roundedView)
        ...
        if classTableView == tableView {
            ...
            HSAddressNetworkHandler().getAddressForPlaceID(placeID: theAddress.placeId, onComplete: {(aAddress, error) in
                if error == nil{
                    ...
                    self.reverseGeoCode((aAddress?.location)!)
                }else{
                    HSLoadingView.standardLoading().stopLoading()
                    HSUtility.showMessage(string: (error?.message)!)
                }
