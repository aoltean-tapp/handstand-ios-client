...
class HSPersonalInfoController: HSBaseController {
    ...
    public var navType:navBarType = .type_None
    ...
    override func viewDidLoad() {
        ...
        if HSUserManager.shared.isUserLoggedIn() {
            if let gIdx = HSUserManager.shared.currentUser?.gender {
                let gView = genderViews[gIdx]
                self.viewToHighlight(gView)
            }

            if let dob = HSUserManager.shared.currentUser?.dateOfBirth {
                let dateFormater = DateFormatter()
                dateFormater.dateFormat = "yyyy-MM-dd"
                selectedDate = dateFormater.date(from: dob)
                let values:(day:Int,month:String,year:Int) = selectedDate!.getDateComponents()
