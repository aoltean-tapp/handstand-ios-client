...
class HSChoosePlanController: HSBaseController {
    ...
    override func viewDidLoad() {
        ...
        if HSUserManager.shared.isUserLoggedIn() == true {
    ...
    @objc func didBuySubscription() {
        if HSUserManager.shared.isUserLoggedIn() == true {
            completedTheIAPPurchase()
        }else {
            HSLoadingView.standardLoading().stopLoading()
            ...
            HSUserNetworkHandler().saveIAPRecepit(forReceiptID: iapReceipt , withSubcription: HSAppManager.shared.handPickedProductIdentifier!, completionHandler: {(success, error) in
                ...
                if success {
                    ...
                    if user?.gender == nil {
                        ...
                        self.navigationController?.pushViewController(personInfoScreen, animated: true)
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if self.dataSource != nil {
            if section == 0 {
                ...
                let h_height:CGFloat = ScreenCons.SCREEN_WIDTH <= 375 ? 80 : 50
...
extension HSChoosePlanController:HSPaymentSuccessProtocol {
    func didTapPaymentSuccess() {
        ...
        let user = HSUserManager.shared.currentUser
        if user?.gender == nil {
            let personalInfoScreen = HSPersonalInfoController()
