...
//        }
        if let theImgURL = avatar {
            avatar = theImgURL.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)
        }

        if let theValue = data.value(forKey: "color") as? String {
            color = theValue
        }
    ...
    init(withUpcomingData data : NSDictionary) {
        ...
        if let theValue =  data.value(forKeyPath: "trainer.color") as? String {
            color = theValue
        }
...
//        }
        if let theValue = data.value(forKeyPath: "trainer.avatarUrl") as? String {
            avatar = theValue.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)
        }
    ...
    func partialTrainerName() -> String {
        ...
        if self.last_name.count > 0 {
            partialName = partialName! + " " + "\(self.last_name.first!)."
        }
        return partialName!
    }
}
