...
class HSUserNetworkHandler: HSNetworkHandler {
    ...
    func updatePassword(forEmailID emailID : String, withPassword password : String, onComplete completionHandler: @escaping (Bool, HSError?) -> ()) {
        ...
        self.request("\(base_url)v3/profile", method: .post, parameters: parameters).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
    ...
    func getNewSubscriptionPlansData(completion:@escaping (AsyncResult<HSGetNewSubscriptionsResponse>)->()) {
        self.request("\(base_url)v3/subscriptionpopup", method: .get, parameters: [:]).responseJSON { response in
            var error : HSError? = nil
