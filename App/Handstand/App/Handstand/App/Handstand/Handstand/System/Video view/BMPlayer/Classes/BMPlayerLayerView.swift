...
open class BMPlayerLayerView: UIView {
    ...
    /// 是否为全屏
    var isBackgroundPlayback  = false
    /// 是否锁定屏幕方向
    ...
    func setupLockScreen(){
        ...
        commandCenter.playCommand.addTarget {event in
            self.play()
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: kMCAudioDidPlay), object: nil)
            return .success
        }
        commandCenter.pauseCommand.isEnabled = true
        commandCenter.pauseCommand.addTarget {event in
            self.pause()
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: kMCAudioDidPause), object: nil)
            return .success
        }
        MPNowPlayingInfoCenter.default().nowPlayingInfo = [
            MPMediaItemPropertyTitle: audioTitle ,
//            MPMediaItemPropertyAlbumTitle: "Handstand",
            MPMediaItemPropertyArtist: "Handstand",
...
//            MPNowPlayingInfoPropertyElapsedPlaybackTime: audioPlayer.progress
    ...
    func setAudioTotalDuration(_ duration : TimeInterval) {
        if audioDuration == 0 {
            MPNowPlayingInfoCenter.default().nowPlayingInfo?[MPMediaItemPropertyPlaybackDuration] = audioDuration
        }
    }
