...
- (NSString*)timeFromNow
{
    ...
    NSTimeInterval delta = -1*[[NSDate date] timeIntervalSinceDate:self];
    NSDateFormatter *theDateFormater = [[NSDateFormatter alloc]init];
    [theDateFormater setDateFormat:@"MMM,d yyyy"];
    NSString *theDateStr = [NSString stringWithFormat:@"%@",[theDateFormater stringFromDate:[NSDate date]]];
    [theDateFormater setDateFormat:@"MMM,d yyyy hh:mm a"];
    theDateStr = [NSString stringWithFormat:@"%@ 11:59 PM",theDateStr];
    NSInteger hoursLeftForToday = [[NSDate date] hoursBeforeDate:[theDateFormater dateFromString:theDateStr]];
    if (delta < 0) {
        return [NSString stringWithFormat:@"%@",[theDateFormater stringFromDate:self]];
    }
    else if (delta < 1 * MINUTE)
    {
        return delta == 1 ? @"Second" : [NSString stringWithFormat:@"%d seconds", (int)delta];
    }
    ...
    else if (delta < hoursLeftForToday * HOUR)
    {
        [theDateFormater setDateFormat:@"hh:mm a"];
//        int hours = floor((double)delta/HOUR);
        return [NSString stringWithFormat:@"Today"];
    }
    else if (delta < (hoursLeftForToday+24) * HOUR)
    {
        return [NSString stringWithFormat:@"Tomorrow"];
    }
    else{
        [theDateFormater setDateFormat:@"MMM,d"];
        return [NSString stringWithFormat:@"on %@",[theDateFormater stringFromDate:self]];
    }
...
+(NSString *)dateStringForShortDate:(NSString *)aSting{
    NSDateFormatter *theDateFormater = [[NSDateFormatter alloc]init];
    [theDateFormater setDateFormat:@"d MMMMM yyyy"];
    NSDate *theDate = [theDateFormater dateFromString:aSting];
    if ([theDate isToday]) {
        return @"Today";
    }
...
+(int)futureDateIndexInArray:(NSArray *)aDates{
    NSDateFormatter *theDateFormater = [[NSDateFormatter alloc]init];
    [theDateFormater setDateFormat:@"d MMMMM yyyy"];
    for (int i =0; i<aDates.count; i++) {
        NSString *theDate = [aDates objectAtIndex:i];
        NSDate *theDateObj = [theDateFormater dateFromString:theDate];        
        if ([theDateObj isFutureDate] || [theDateObj isToday]) {
            return i;
        }
    }
...
-(NSString *)remainingDaysFromToday{
    NSDate *theDate = NSDate.date;
    NSDate *theOtherDate = self;
    NSInteger numberOfDays =  [theOtherDate daysAfterDate:theDate];
    return [NSString stringWithFormat:@"%ld", (long)numberOfDays];
}


-(NSString *)remainingTimeForToday{
    NSDate *theDate = NSDate.date;
    NSDateFormatter *formater = [[NSDateFormatter alloc] init];
    formater.dateFormat = @"d-MM-yyyy";
    NSString *dateStr = [formater stringFromDate:theDate];
    dateStr = [NSString stringWithFormat: @"%@ 11:59:59 pm", dateStr];
    formater.dateFormat = @"d-MM-yyyy hh:mm:ss a";
    NSDate *theOtherDate = [formater dateFromString:dateStr];
    return [self timeIntervalToString:theOtherDate];
}


-(NSString *)timeIntervalToString:(NSDate *)givenDate
{
    NSDate *theDate = NSDate.date;
    NSTimeInterval interval = [givenDate timeIntervalSinceDate: theDate];
