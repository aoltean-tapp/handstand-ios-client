...
class HSSignupController: HSBaseController, HSValidationTextfieldDelegate, UGLinkLabelDelegate {
    ...
    @IBAction func onCompleteAction(_ sender: HSLoadingButton) {
        ...
        if isAllFieldValid() {
            sender.startLoading()
            var policyState = ""
            if policyCheck.isHidden == false {
                policyState = "on"
            }
            //HSLoadingView.standardLoading().startLoading()
            HSUserNetworkHandler().signup(firstNameTextfield.text()!, lastname: lastNameTextfield.text()!, mobile: phoneNumberTextfield.text()!, email: emailTextfield.text()!, password: passwordTextfield.text()!, inputData: ["rebook_accept" : policyState], completionHandler: { (success, error) in
                if success {
