...
class HSUserNetworkHandler: HSNetworkHandler {
    func login(_ email: String, password: String, completionHandler: @escaping (Bool, HSError?) -> ()) -> () {
        ...
        self.request("\(url)login", method: .get, parameters: parameters).responseJSON { response in
    ...
    func signup(_ firstname: String, lastname: String, mobile: String, email: String, password: String, inputData : NSDictionary?, completionHandler: @escaping (Bool, HSError?) -> ()) -> () {
        //        let parameters = ["firstname" : firstname, "lastname" : lastname, "mobile" : mobile, "type" : "User", "email" : email, "password" : password, "agree" : (inputData["agree"])!, "rebook_accept" : (inputData["rebook_accept"])!]
        let parameters = ["firstname" : firstname, "lastname" : lastname, "mobile" : mobile, "type" : "User", "email" : email, "password" : password, "agree" : "1", "rebook_accept" : "\((inputData?["rebook_accept"])!)"]
    ...
    func getUserProfile(completionHandler: @escaping (Bool, HSError?) -> ()) -> () {
        self.request("\(url)profile", method: .get, parameters: [:]).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                let result = (JSON as AnyObject).value(forKey: "result") as! String
                var success = false
            ...
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
                        ...
                        if let colorAttribute = (JSON as AnyObject).value(forKey: "color_attribute") as? NSDictionary {
                            if colorAttribute.count > 0 {
                                HSAppManager.shared.colorAttribute = colorAttribute["color"] as? String
                            }
                        }
                        completionHandler(true, error)
                        return
    ...
    func saveProfileInfo( profileData : Dictionary<String,String>?, completionHandler: @escaping (Bool, HSError?) -> ())  {
        self.request("\(url)profile", method: .post, parameters: profileData).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
    ...
    func saveIAPRecepit(forReceiptID receiptID : String, withSubcription subcription : String,  completionHandler: @escaping (Bool, HSError?) -> ())  {
        ...
        self.request("\(url)savemembership", method: .post, parameters: parameters).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
                        //                        if let iapPeriod = HSController.defaultController.iapPeriod {
                        //                            HSAnalytics.setEventForTypes(types: analyticsType, withLog: kAEUserSubscription, withData: [kAELabel :iapPeriod , kGAEventsCategoryName : kAEUserCategory])
                    }
                    return completionHandler(true, error)
    ...
    func validateMembership(_ completionHandler: @escaping (Bool, HSError?) -> ()) {
        self.request("\(url)validatemembership", method: .get, parameters: nil).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
    ...
    func forgotPassword(forEmailID emailID : String, onComplete completionHandler: @escaping (Bool, HSError?) -> ()) {
        ...
        self.request("\(url)smscode2", method: .get, parameters: parameters).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
    ...
    func updatePassword(forEmailID emailID : String, withPassword password : String, onComplete completionHandler: @escaping (Bool, HSError?) -> ()) {
        let parameters = ["email" : emailID, "password":password]
        self.request("\(url)changepasswordviaemail", method: .post, parameters: parameters).responseJSON { response in
            var error : HSError? = nil
            if let JSON = response.result.value {
                if let result = (JSON as AnyObject).value(forKey: "result") as? String{
                    if result == "success" {
    ...
    func requestPhneVerificationCode(with verb:String,
        ...
        var b_url:String = ""
            ...
            b_url = "https://handstandapp.com/api/"
        #elseif HandstandStage
            b_url = "https://stage.handstandapp.com/api/"
        #endif
       // https://stage.handstandapp.com/api/verification?action=generate&mobile=4244420933
        self.request("\(b_url)verification?"+subRoute, method: .get, parameters: [:]).responseJSON { response in
            var result:String?
            var message:String?
